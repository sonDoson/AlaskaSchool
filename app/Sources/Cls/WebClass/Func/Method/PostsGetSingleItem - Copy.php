...
class PostsGetSingleItem{
    ...
    public static function postsGetSingleItemForList($table_posts, $id_posts){ 
        //get data
        $data_posts = DB::table($table_posts)
            ->select('id', 'id_category', 'name_en', 'name_vn', 'created_at')
        ...
        $image = array();
        $images = DB::table($table_posts . '_images')->where('id_posts', $id_posts)->get();
        if(empty($images->id_posts)){
            foreach($images as $key => $value){
                $image[] = $value->image_path;
                //thumbnail path
                if($key == 0){
                    $image['thumbnail'] = str_replace(
                        substr($value->image_path, strpos($value->image_path, '.')),
                        "_resize" . substr($value->image_path, strpos($value->image_path, '.')), 
                        $value->image_path
                    );
                }
            }
        }   else    {
            $image[0] = '/uploads/logo/background.png';
            $image['thumbnail'] = '/uploads/logo/background.png';
        }
        //check stress
        $stress = DB::table($table_posts . '_stress')->where('id_posts', $id_posts)->first();
        if(!empty($stress->id_posts)){
